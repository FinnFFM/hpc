...
// Kraftberechnung
static void compute_forces_simd_omp(BodySoA& bodies, float G, float soft) {
  
  size_t n = bodies.n;
  size_t np = bodies.n_padded;

  // Parallelisiert die äußere Schleife über die Körper
...
static void kick_drift_simd_omp(BodySoA& bodies, float dt) {
  ...
  // Für jeden Körper (Vektorisiert) und Parallelisiert
...
static void kick_simd_omp(BodySoA& bodies, float dt)
{
    size_t np = bodies.n_padded;
    const float half_dt = 0.5f * dt;
...
static int run_simd_omp(int argc, char** argv) {
  // Initialisierung (SOA Layout, Anfangskräfte)
  BodySoA bodies(n_bodies);
  init_bodies_soa(bodies);
  compute_forces_simd_omp(bodies, G, softening);

  if (argc >= 2 && std::string(argv[1]) == "benchmark") {
    auto t0 = std::chrono::high_resolution_clock::now();
    for (int s = 0; s < benchmark_steps; ++s) {
      kick_drift_simd_omp(bodies, dt);
      compute_forces_simd_omp(bodies, G, softening);
      kick_simd_omp(bodies, dt);
    }
    ...
    if (result) {
      ...
      std::cout << "[simd_omp] steps=" << benchmark_steps
                << " time_ms=" << ms
  ...
  sf::RenderWindow window(sf::VideoMode(WIDTH, HEIGHT), "N-Body (simd_omp)");
...
static BackendRegister reg("simd-omp", run_simd_omp);